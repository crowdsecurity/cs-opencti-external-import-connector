...
| `CONNECTOR_DURATION_PERIOD`                    | No        |         | Interval between two imports in ISO-8601 format<br />Default: `PT24H` (24 hours) |
...
| `CROWDSEC_IMPORT_QUERY_SINCE`                  | No        | Number  | ime window in hours for fetching CrowdSec data. Only IPs created within the last N hours are retrieved. <br />Default: `24` |
...
| `CROWDSEC_TLP_LEVEL`                           | No        | String  | TLP for created STIX objects (observable, etc). Possible values are: `clear`, `white`, `green`, `amber`, `amber+strict`, `red` . If an observable already exists, its TLP will be left unchanged.<br />Default: `amber` |
